...
// Get platform breakdown for unique reviews
function getPlatformBreakdown(data) {
    const uniqueReviews = {};

    data.forEach(row => {
        if (!uniqueReviews[row.review_id]) {
            uniqueReviews[row.review_id] = row.platform;
        }
    });

    const platformCounts = {};
    Object.values(uniqueReviews).forEach(platform => {
        platformCounts[platform] = (platformCounts[platform] || 0) + 1;
    });

    return {
        total: Object.keys(uniqueReviews).length,
...
function updateKPIs(data) {
    ...
    const platformData = getPlatformBreakdown(data);
    ...
    // Update Total Reviews KPI card
    document.getElementById('totalReviewsKPI').textContent = platformData.total.toLocaleString();

    // Create platform breakdown text
    const platformBreakdown = Object.entries(platformData.platforms)
        .map(([platform, count]) => `${platform.charAt(0).toUpperCase() + platform.slice(1)}: ${count.toLocaleString()}`)
        .join(' | ');
    document.getElementById('platformBreakdown').textContent = platformBreakdown;

    // Update footer with detailed stats
    const footerText = `Total: ${platformData.total.toLocaleString()} reviews | ${platformBreakdown}`;
    document.getElementById('footerPlatformStats').textContent = footerText;

    // Keep legacy totalReviews for backward compatibility
    document.getElementById('totalReviews').textContent = platformData.total.toLocaleString();
}
